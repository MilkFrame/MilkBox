data = {
    ["LobbyGadgetMeta"] = {
        ["halloween_2024_spider"] = {
            ["image"] = "rbxassetid://126845989227505",
            ["name"] = "Halloween 2024 Spider",
            ["pet"] = "default_spider"
        },
        ["headhunter"] = {
            ["items"] = {
                {["itemType"] = "headhunter", ["amount"] = 1},
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "arrow", ["amount"] = 4}
            },
            ["image"] = "rbxassetid://13421692306",
            ["name"] = "Headhunter"
        },
        ["flying_backpack"] = {
            ["items"] = {{["itemType"] = "flying_backpack", ["amount"] = 1}},
            ["image"] = "rbxassetid://13630754419",
            ["name"] = "Flying Backpack"
        },
        ["invisibility_potion"] = {
            ["items"] = {
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "invisibility_potion", ["amount"] = 1}
            },
            ["image"] = "rbxassetid://7836794914",
            ["name"] = "Invisibility Potion"
        },
        ["snow_ball"] = {
            ["items"] = {{["refillable"] = true, ["itemType"] = "snowball", ["amount"] = 3}},
            ["battlepassSeason"] = "season3",
            ["name"] = "Snow Ball",
            ["unlockedInBattlePass"] = true
        },
        ["cutlass_ghost"] = {
            ["items"] = {{["itemType"] = "cutlass_ghost", ["amount"] = 1}},
            ["image"] = "rbxassetid://10729541018",
            ["name"] = "Ghost Cutlass"
        },
        ["can_of_beans"] = {
            ["items"] = {{["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "can_of_beans", ["amount"] = 1}},
            ["image"] = "rbxassetid://13918757728",
            ["name"] = "Can of Beans"
        },
        ["lasso"] = {
            ["items"] = {{["itemType"] = "lasso", ["amount"] = 1}},
            ["image"] = "rbxassetid://7192710930",
            ["name"] = "Lasso"
        },
        ["hot_air_balloon"] = {
            ["items"] = {{["refillable"] = true, ["itemType"] = "hot_air_balloon_deploy", ["amount"] = 1}},
            ["image"] = "rbxassetid://13701861348",
            ["name"] = "Hot Air Balloon"
        },
        ["mythic_great_hammer"] = {
            ["items"] = {{["itemType"] = "mythic_great_hammer", ["amount"] = 1}},
            ["image"] = "rbxassetid://13832631998",
            ["name"] = "Paragon"
        },
        ["toy_hammer"] = {["name"] = "Toy Hammer", ["items"] = {{["itemType"] = "toy_hammer", ["amount"] = 1}}},
        ["summer_2024_turtle"] = {
            ["image"] = "rbxassetid://18460677701",
            ["name"] = "Summer 2024 Turtle",
            ["pet"] = "turtle"
        },
        ["jump_boots"] = {
            ["items"] = {{["itemType"] = "jump_boots"}},
            ["battlepassSeason"] = "season3",
            ["name"] = "Jump Boots",
            ["unlockedInBattlePass"] = true
        },
        ["mythic_dagger"] = {
            ["items"] = {{["itemType"] = "mythic_dagger", ["amount"] = 1}},
            ["image"] = "rbxassetid://13832903272",
            ["name"] = "Deathbloom"
        },
        ["tennis_racket"] = {
            ["items"] = {
                {["itemType"] = "tennis_racket", ["amount"] = 1},
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "tennis_ball", ["amount"] = 4}
            },
            ["image"] = "rbxassetid://10392204924",
            ["name"] = "Tennis Racket"
        },
        ["grappling_hook"] = {
            ["name"] = "Grappling Hook",
            ["items"] = {{["itemType"] = "grappling_hook", ["amount"] = 1}}
        },
        ["raven"] = {
            ["items"] = {{["refillable"] = true, ["itemType"] = "raven", ["amount"] = 1}},
            ["battlepassSeason"] = "season4",
            ["name"] = "Raven",
            ["unlockedInBattlePass"] = true
        },
        ["laser_sword"] = {
            ["items"] = {{["itemType"] = "laser_sword", ["amount"] = 1}},
            ["image"] = "rbxassetid://11775182286",
            ["name"] = "Laser Sword"
        },
        ["necromancer_staff"] = {
            ["items"] = {{["itemType"] = "necromancer_staff", ["amount"] = 1}},
            ["image"] = "rbxassetid://11350214469",
            ["name"] = "Necromancer Staff"
        },
        ["rainbow_staff"] = {
            ["items"] = {{["itemType"] = "rainbow_staff", ["amount"] = 1}},
            ["image"] = "rbxassetid://12813669578",
            ["name"] = "Mirage Staff"
        },
        ["summer_2024_crab"] = {["image"] = "rbxassetid://18235888419", ["name"] = "Summer 2024 Crab", ["pet"] = "crab"},
        ["jellyfish_mount"] = {
            ["items"] = {{["refillable"] = true, ["itemType"] = "jellyfish_mount_deploy", ["amount"] = 1}},
            ["image"] = "rbxassetid://18129974979",
            ["name"] = "Jellyfish Mount"
        },
        ["mythic_gauntlets"] = {
            ["items"] = {{["itemType"] = "mythic_gauntlets", ["amount"] = 1}},
            ["image"] = "rbxassetid://14839096268",
            ["name"] = "Warfists"
        },
        ["elk_mount"] = {["mount"] = "elk", ["image"] = "rbxassetid://15716032069", ["name"] = "Elk Mount"},
        ["grim_reaper_scythe"] = {
            ["items"] = {{["itemType"] = "reaper_scythe", ["amount"] = 1}},
            ["image"] = "rbxassetid://17768761460",
            ["name"] = "Grim Reaper Scythe"
        },
        ["beachball"] = {
            ["items"] = {{["itemType"] = "beachball", ["amount"] = 1}},
            ["image"] = "rbxassetid://18149456734",
            ["name"] = "Beachball"
        },
        ["leather_helmet"] = {
            ["items"] = {{["itemType"] = "leather_helmet", ["amount"] = 1}},
            ["image"] = "rbxassetid://6855466216",
            ["name"] = "Leather Helmet"
        },
        ["party_hat_launcher"] = {
            ["items"] = {
                {["itemType"] = "party_hat_launcher", ["amount"] = 1},
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "party_hat_missile", ["amount"] = 2}
            },
            ["image"] = "rbxassetid://17580323633",
            ["name"] = "Party Hat Launcher"
        },
        ["void_sword"] = {["name"] = "Void Sword", ["items"] = {{["itemType"] = "void_sword", ["amount"] = 1}}},
        ["dodo_bird"] = {["mount"] = "dodo_bird", ["image"] = "rbxassetid://11149949979", ["name"] = "Dodo Bird"},
        ["none"] = {["name"] = "None"},
        ["nocturne"] = {
            ["items"] = {{["itemType"] = "mythic_scythe", ["amount"] = 1}},
            ["image"] = "rbxassetid://13832902921",
            ["name"] = "Nocturne"
        },
        ["angel_wings"] = {
            ["items"] = {{["itemType"] = "angel_wings", ["amount"] = 1}},
            ["image"] = "rbxassetid://17192935133",
            ["name"] = "Angel Wings"
        },
        ["ares_spear"] = {
            ["items"] = {{["refillCooldown"] = 3, ["refillable"] = true, ["itemType"] = "spear", ["amount"] = 1}},
            ["image"] = "rbxassetid://7808151805",
            ["name"] = "Ares Spear"
        },
        ["diamond_chakram"] = {
            ["items"] = {{["itemType"] = "ninja_chakram_3", ["amount"] = 1}},
            ["image"] = "rbxassetid://15515023612",
            ["name"] = "Diamond Chakram"
        },
        ["jade_hammer"] = {
            ["items"] = {{["itemType"] = "jade_hammer"}},
            ["battlepassSeason"] = "season4",
            ["name"] = "Jade Hammer",
            ["unlockedInBattlePass"] = true
        },
        ["FIREWORK_ROCKET_LAUNCHER"] = {
            ["items"] = {
                {["itemType"] = "firework_rocket_launcher", ["amount"] = 1},
                {
                    ["refillCooldown"] = 8,
                    ["refillable"] = true,
                    ["itemType"] = "firework_rocket_missile",
                    ["amount"] = 2
                }
            },
            ["image"] = "rbxassetid://15798141956",
            ["name"] = "Firework Rocket Launcher"
        },
        ["flying_broom"] = {
            ["items"] = {{["refillable"] = true, ["itemType"] = "flying_broom_deploy", ["amount"] = 1}},
            ["image"] = "rbxassetid://15115405598",
            ["name"] = "Flying Broom"
        },
        ["mending_canopy_staff_tier_3"] = {
            ["items"] = {{["itemType"] = "mending_canopy_staff_tier_3", ["amount"] = 1}},
            ["image"] = "rbxassetid://17007892915",
            ["name"] = "Nyoka Staff"
        },
        ["spirit_assassin_dagger"] = {
            ["items"] = {{["itemType"] = "spirit_dagger", ["amount"] = 1}},
            ["image"] = "rbxassetid://16385255903",
            ["name"] = "Spirit Dagger"
        },
        ["snowball_launcher"] = {
            ["items"] = {
                {["itemType"] = "snowball_launcher", ["amount"] = 1},
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "snowball", ["amount"] = 4}
            },
            ["image"] = "rbxassetid://15628201582",
            ["name"] = "Snowball Launcher"
        },
        ["hunters_echo"] = {
            ["items"] = {
                {["refillCooldown"] = 10, ["refillable"] = true, ["itemType"] = "hunters_echo", ["amount"] = 1}
            },
            ["image"] = "rbxassetid://14978481226",
            ["name"] = "Hunter's Echo"
        },
        ["blunderbuss"] = {
            ["items"] = {{["itemType"] = "blunderbuss", ["amount"] = 1}},
            ["image"] = "rbxassetid://10722841562",
            ["name"] = "Blunderbuss"
        },
        ["falconer_bow"] = {
            ["items"] = {
                {["itemType"] = "falconer_bow", ["amount"] = 1},
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "arrow", ["amount"] = 4}
            },
            ["image"] = "rbxassetid://17014870717",
            ["name"] = "Bekzat Bow"
        },
        ["rageblade"] = {
            ["items"] = {{["itemType"] = "rageblade"}},
            ["battlepassSeason"] = "season3",
            ["name"] = "Rageblade",
            ["unlockedInBattlePass"] = true
        },
        ["frosty_hammer"] = {
            ["items"] = {{["itemType"] = "frosty_hammer", ["amount"] = 1}},
            ["image"] = "rbxassetid://11831565831",
            ["name"] = "Frosty Hammer"
        },
        ["cloak"] = {
            ["items"] = {{["itemType"] = "cloak", ["amount"] = 1}},
            ["image"] = "rbxassetid://18952530979",
            ["name"] = "Cloak"
        },
        ["sky_scythe"] = {
            ["items"] = {{["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "sky_scythe", ["amount"] = 1}},
            ["image"] = "rbxassetid://13629036006",
            ["name"] = "Sky Scythe"
        },
        ["grenade_launcher"] = {
            ["name"] = "Grenade Launcher",
            ["items"] = {
                {["itemType"] = "grenade_launcher", ["amount"] = 1},
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "stun_grenade", ["amount"] = 5}
            }
        },
        ["light_sword"] = {["name"] = "Light Sword", ["items"] = {{["itemType"] = "light_sword", ["amount"] = 1}}},
        ["freiya_ice_sword"] = {
            ["items"] = {{["itemType"] = "ice_sword"}},
            ["battlepassSeason"] = "season4",
            ["name"] = "Freiya Ice Sword",
            ["unlockedInBattlePass"] = true
        },
        ["anniversary_balloons"] = {
            ["items"] = {
                {["refillCooldown"] = 8, ["refillable"] = true, ["itemType"] = "anniversary_balloon", ["amount"] = 3}
            },
            ["image"] = "rbxassetid://17580323788",
            ["name"] = "Anniversary Balloons"
        },
        ["glitch_guitar"] = {
            ["items"] = {{["itemType"] = "glitch_guitar", ["amount"] = 1}},
            ["image"] = "rbxassetid://12509567989",
            ["name"] = "Glitch Guitar"
        },
        ["rocket_belt"] = {["name"] = "Rocket Belt", ["items"] = {{["itemType"] = "rocket_belt", ["amount"] = 1}}},
        ["scythe"] = {["name"] = "Scythe", ["items"] = {{["itemType"] = "scythe", ["amount"] = 1}}},
        ["dino_mount"] = {["mount"] = "dino", ["image"] = "rbxassetid://9855535867", ["name"] = "Dino Mount"},
        ["impulse_gun"] = {
            ["items"] = {{["itemType"] = "impulse_gun", ["amount"] = 1}},
            ["image"] = "rbxassetid://13629029360",
            ["name"] = "Impulse Gun"
        },
        ["VOID_AXE"] = {
            ["items"] = {{["itemType"] = "void_axe", ["amount"] = 1}},
            ["image"] = "rbxassetid://8322058718",
            ["name"] = "Void Axe"
        },
        ["barbarian_helmet"] = {
            ["items"] = {{["itemType"] = "barbarian_helmet", ["amount"] = 1}},
            ["image"] = "rbxassetid://14559460074",
            ["name"] = "Barbarian Helmet"
        },
        ["twirl_blade"] = {["name"] = "Twirl Blade", ["items"] = {{["itemType"] = "twirlblade", ["amount"] = 1}}},
        ["hang_glider"] = {
            ["name"] = "Hang Glider",
            ["items"] = {{["refillable"] = true, ["itemType"] = "hang_glider", ["amount"] = 1}}
        },
        ["infernal_saber"] = {
            ["name"] = "Infernal Saber",
            ["items"] = {{["itemType"] = "infernal_saber", ["amount"] = 1}}
        },
        ["speed_boots"] = {
            ["items"] = {{["itemType"] = "speed_boots"}},
            ["battlepassSeason"] = "season3",
            ["name"] = "Speed Boots",
            ["unlockedInBattlePass"] = true
        }
    }
}
